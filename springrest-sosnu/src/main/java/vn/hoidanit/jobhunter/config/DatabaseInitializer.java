...
public class DatabaseInitializer implements CommandLineRunner {
    ...
    private void configureCharacterEncoding() {
        log.info(">>> STARTING CONFIGURE CHARACTER ENCODING...");
        try {
            ...
            for (String sql : sqlCommands) {
                try {
                    ...
                    log.warn("Skipping command due to error (might be expected on first run): {}", e.getMessage());
                }
            }

            log.info(">>> CONFIGURE CHARACTER ENCODING SUCCESS!");
        } catch (Exception e) {
            log.error(">>> FAILED TO CONFIGURE CHARACTER ENCODING: {}", e.getMessage());
        }
    }

    // TẠO MỘT HÀM MỚI ĐỂ CHẠY SQL
    private void initFullTextIndex() {
        log.info(">>> STARTING CREATE FULLTEXT INDEXES...");
        try {
            ...
            for (String sql : sqlCommands) {
                log.info("Executing: {}", sql); // In ra lệnh đang chạy để dễ theo dõi
                jdbcTemplate.execute(sql);
            }

            log.info(">>> CREATE FULLTEXT INDEXES SUCCESS!");
        } catch (Exception e) {
    ...
    public void run(String... args) throws Exception {
        ...
        if (countPermissions == 0) {
            ...
            arr.add(new Permission("get candidate users for recruiter", "/api/v1/gemini/candidate",
                    "POST", "SearchUsersAI"));

            arr.add(new Permission("mock interview by user", "/api/v1/gemini/mock-interview",
                    "POST", "GeminiMockInterview"));
            arr.add(new Permission("evaluate-interview by user",
                    "/api/v1/gemini/evaluate-interview", "POST", "GeminiMockInterview"));
            arr.add(new Permission("evaluate company by user",
                    "/api/v1/gemini/evaluate-company/{id}", "POST", "GeminiEvaluateCompany"));
