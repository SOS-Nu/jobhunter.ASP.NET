...
{
    /// <summary>
    /// Maps from: vn.hoidanit.jobhunter.service.UserService
    /// </summary>
    ...
    public class UserService : IUserService
    {
        ...
        public async Task DeleteUserAsync(long id)
        {
            ...
            if (user != null)
            {
                // Delete chat messages and rooms first (matching Spring Boot cascading logic)
        ...
        {
            var user = await _context.Users.FindAsync(userPayload.Id);
            if (user != null)
            {
                user.Status = Enums.UserStatusEnum.ONLINE;
                await _context.SaveChangesAsync();
            }
        }
        ...
        public async Task DisconnectAsync(User userPayload)
        {
            ...
            if (user != null)
            {
                user.Status = Enums.UserStatusEnum.OFFLINE;
                await _context.SaveChangesAsync();
            }
        }

        // ============================================================
        // MISSING METHODS FROM JAVA - ADDED FOR FUNCTIONAL PARITY
        // ============================================================

        /// <summary>
        /// Maps from: UserService.enforceSessionLimitAndKickOldest(User user)
        /// Instead of rejecting login, kicks the OLDEST active session.
        /// </summary>
        public async Task EnforceSessionLimitAndKickOldestAsync(User user)
        {
            ...
            // 3. If at or over limit, kick oldest
            if (activeCount >= MaxSessionsPerUser)
            {
                _logger.LogWarning("User {Email} (Verified Login) đã đạt giới hạn session. Đang xóa session cũ nhất...", user.Email);
        ...
        /// <summary>
        /// Maps from: UserService.deleteAllOtherSessions(long userId, String currentJti)
        ...
        /// <summary>
        /// Maps from: UserService.deleteSessionsByIds(List<Long> sessionIds, long userId)
        /// Batch delete sessions by IDs, scoped to user.
        /// </summary>
        ...
        /// <summary>
        /// Maps from: UserService.uploadMainResume(MultipartFile file)
        /// Upload CV file and set it as user's main resume.
        /// </summary>
        public async Task<ResUploadFileDTO> UploadMainResumeAsync(IFormFile file)
        {
            ...
            var uploadedFileName = await _fileService.StoreAsync(file, folder);
        ...
        /// <summary>
        /// Maps from: UserService.getPermissionKeysByEmail(String email)
        /// Returns permission keys in format "METHOD:/path" for authorization checks.
        /// </summary>
        ...
        /// <summary>
        /// Maps from: UserService.getLastSecurityUpdateAt(String email)
        /// Returns epoch millis for token validation.
        /// </summary>
        public async Task<long?> GetLastSecurityUpdateAtAsync(string email)
        {
            ...
            var user = await _context.Users
                ...
                .Select(u => new { u.LastSecurityUpdateAt })
                .FirstOrDefaultAsync();

            if (user?.LastSecurityUpdateAt == null) return null;

            return new DateTimeOffset(user.LastSecurityUpdateAt.Value, TimeSpan.Zero).ToUnixTimeMilliseconds();
        }

        /// <summary>
        /// Maps from: UserService.changePassword(String email, String newPassword)
        /// Encode password + update security timestamp to invalidate old access tokens.
        /// </summary>
        ...
        /// <summary>
        /// Maps from: UserService.canSubmitCv(String email)
        /// CV limit check: 10 for normal users, 20 for VIP. Auto-deactivates expired VIP.
        /// </summary>
        ...
        /// <summary>
        /// Maps from: UserService.incrementCvSubmission(String email)
        /// Increment CV submission counter after successful resume creation.
        /// </summary>
