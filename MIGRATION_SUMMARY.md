...
## 2. Key Business Logic Migrated (Functional Parity)
...
## 4. Pending / Next Steps for New AI Model
...
3.  **Real-time Chat:** Ensure `ChatService` (SignalR) is fully aligned with the Java WebSocket logic if not already completed.
4.  **Unit Testing:** Implement XUnit tests for the `ResumeService` quota logic as it is critical for monetization.

---

## 5. Coding Principles to Maintain
